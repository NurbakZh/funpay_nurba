...
NAME = "Xbox Accounts АРЕНДА Plugin"
VERSION = "0.0.1"
DESCRIPTION = "Данный плагин позволяет управлять системой аренды аккаунтов на Xbox"
CREDITS = "@nurba_zh"
SETTINGS_PAGE = False
UUID = "f5b3b3b4-0b3b-4b3b-8b3b-0b3b3b3b3b7b"

logger = getLogger("FPC.xbox_accounts_plugin")
RUNNING = False
...
def init_commands(cardinal: Cardinal):
        ...
        try:
            ...
            if game_options is not None:
                ...
                if suitable_game_option is None:
                    suitable_game_option = next((option for option in game_options if option["text"] in ["Xbox", "Xbox Series X", "Xbox Series S", "Xbox One"]), None)

            if platform_options is not None:
                suitable_platform_option = next((option for option in platform_options if option["text"] in ["Xbox", "Xbox Series X", "Xbox Series S", "Xbox One"]), None)
                if not suitable_platform_option:
                    raise Exception(f"No suitable platform option found for 'Xbox'")
    ...
    def handle_add_account(message: Message):
        msg = bot.send_message(message.chat.id, "📧 Введите логин аккаунта Xbox:")
        bot.register_next_step_handler(msg, process_login_step)

    def process_login_step(message: Message):
        login = message.text
        msg = bot.send_message(message.chat.id, "🔑 Введите пароль аккаунта Xbox:")
    ...
    def handle_message(call):
        ...
        msg = bot.send_message(call.message.chat.id, "Введите логин аккаунта Xbox:")
