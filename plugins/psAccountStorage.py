...
NAME = "PlayStation Storage Plugin"
VERSION = "0.0.1"
DESCRIPTION = "Плагин для хранения и управления данными PlayStation аккаунтов"
CREDITS = "@nurba_zh"
SETTINGS_PAGE = False
UUID = "f5b3b3b4-0b3b-4b3b-8b3b-0b3b3b3b4b8b"

logger = getLogger("FPC.ps_storage_plugin")

TEMP_STORAGE = {}

ACCOUNT_PAGE = "ps_acc_page"
ACCOUNT_SELECT = "ps_acc_select"
EDIT_ACCOUNT = "ps_edit_acc"
ACCOUNT_VIEW = "ps_acc_view"

ACCOUNTS_PER_PAGE = 5

def load_accounts(filename='storage/plugins/ps_accounts.json') -> Dict:
...
def save_accounts(accounts: Dict, filename='storage/plugins/ps_accounts.json'):
...
def format_account_info(account_data: Dict) -> str:
    ...
    return (f"📋 Данные аккаунта PlayStation:\n\n"
...
def init_commands(cardinal: Cardinal):
    ...
    def handle_add_account(message):
        ...
        msg = cardinal.telegram.bot.send_message(
            message.chat.id,
            "📧 Введите логин аккаунта PlayStation:"
        )
    ...
    def process_additional_info_step(message):
        ...
        # Update password in ps_accounts.json if account exists there
        try:
            with open('storage/plugins/ps_accounts.json', 'r', encoding='utf-8') as f:
            ...
            logger.warning("ps_accounts.json not found, skipping password update in games")
        except Exception as e:
            logger.error(f"Error updating password in ps_accounts.json: {e}")
    ...
    def handle_change_account(message):
        ...
        try:
            ...
            cardinal.telegram.bot.send_message(
                ...
                "Пример: /change_ps_account login123"
            )
    ...
    def handle_get_account(message):
        ...
        try:
            ...
            cardinal.telegram.bot.send_message(
                message.chat.id,
                "❌ Пожалуйста, укажите логин аккаунта после команды.\nПример: /get_ps_account login123"
            )
    ...
    def handle_callback_query(c: CallbackQuery):
        ...
        elif data.startswith("delete_ps_acc_"):
            # Handle account deletion
            login = '_'.join(data.split('_')[2:])  # Get everything after delete_ps_acc_
            accounts = load_accounts()
    ...
    def handle_delete_account(message):
        ...
        try:
            ...
            cardinal.telegram.bot.send_message(
                message.chat.id,
                "❌ Пожалуйста, укажите логин аккаунта после команды.\n"
                "Пример: /delete_ps_account login123"
            )
    ...
    def create_delete_keyboard(accounts: Dict, page: int = 0) -> InlineKeyboardMarkup:
        ...
        for login in current_accounts:
            keyboard.add(InlineKeyboardButton(
                text=f"🗑️ {login}",
                callback_data=f"delete_ps_acc_{login}"
            ))
    ...
    cardinal.add_telegram_commands(UUID, [
        ("add_new_ps_account", "добавить новый PlayStation аккаунт", True),
        ("get_ps_accounts", "показать список аккаунтов", True),
        ("get_ps_account", "получить информацию о конкретном аккаунте", True),
        ("change_ps_accounts", "изменить данные аккаунтов", True),
        ("change_ps_account", "изменить данные конкретного аккаунта", True),
        ("delete_ps_account", "удалить конкретный аккаунт", True),
        ("delete_ps_accounts", "показать список аккаунтов для удаления", True),
    ])

    # Register handlers
    cardinal.telegram.msg_handler(handle_add_account, commands=["add_new_ps_account"])
    cardinal.telegram.msg_handler(handle_get_accounts, commands=["get_ps_accounts"])
    cardinal.telegram.msg_handler(handle_get_account, commands=["get_ps_account"])
    cardinal.telegram.msg_handler(handle_change_account, commands=["change_ps_account"])
    cardinal.telegram.msg_handler(handle_change_accounts, commands=["change_ps_accounts"])
    cardinal.telegram.msg_handler(handle_delete_account, commands=["delete_ps_account"])
    cardinal.telegram.msg_handler(handle_delete_accounts, commands=["delete_ps_accounts"])
    cardinal.telegram.cbq_handler(handle_callback_query, lambda c: c.data.startswith((ACCOUNT_PAGE, ACCOUNT_SELECT, ACCOUNT_VIEW, "delete_ps_acc_")))
